...
router.post("/isbn", async (req, res) => {
  console.log(req.body);
  client.query(
    "SELECT books.isbn, books.title, books.description, books.price, authors.f_name, authors.l_name, authors.publisher, books.genre, books.year_published, books.copies_sold FROM books INNER JOIN authors ON (books.author_id = authors.author_id) WHERE isbn = " +
      req.body.isbn,
    function(err, result) {
      console.log(result);
...
router.post("/author", async (req, res) => {
  console.log(req.body);
  client.query(
    "SELECT books.isbn, books.title, books.description, books.price, authors.f_name, authors.l_name, authors.publisher, books.genre, books.year_published, books.copies_sold FROM books INNER JOIN authors ON (books.author_id = authors.author_id) WHERE f_name = " +
      req.body.f_name +
      " AND l_name = " +
      req.body.l_name +
      " AND publisher = " +
